...
import org.dromara.project.domain.IpOss;
import org.dromara.project.mapper.IpOssMapper;
...
public class IpOssServiceImpl implements IpOssService {
    ...
    public void updateIpOssByIpId(Long ipId, List<Long> ossIdList) {
        ...
        List<IpOss> oldIpOssList =
            ipOssMapper.selectList(new LambdaQueryWrapper<IpOss>().eq(IpOss::getIpId, ipId)).stream()
                .collect(Collectors.toList());
        if (ossIdList == null || ossIdList.isEmpty()) {
        ...
        // 使用 Stream API 计算差集
        List<IpOss> addIpOssList =
            ipOssList.stream().filter(ipOss -> !oldIpOssSet.contains(ipOss)).collect(Collectors.toList());
        List<Long> delIpOssList = oldIpOssList.stream().filter(ipOss -> !ipOssSet.contains(ipOss)).map(IpOss::getOssId)
            .collect(Collectors.toList());
    ...
    public List<SysOssVo> getSysOssVoListByIpId(Long ipId) {
        List<Long> ossIds = ipOssMapper.selectList(new LambdaQueryWrapper<IpOss>().eq(IpOss::getIpId, ipId)).stream()
            .map(IpOss::getOssId).collect(Collectors.toList());
        if (ossIds.isEmpty()) {
