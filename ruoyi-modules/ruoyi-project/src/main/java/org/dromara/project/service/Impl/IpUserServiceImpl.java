...
import org.dromara.project.mapper.IpUserMapper;
...
public class IpUserServiceImpl implements IpUserService {
    ...
    public void updateIpUserByIpId(Long ipId, List<Long> userIdList) {
        ...
        List<IpUser> oldIpUserList =
            ipUserMapper.selectList(new LambdaQueryWrapper<IpUser>().eq(IpUser::getIpId, ipId)).stream()
        ...
        List<IpUser> addIpUserList =
            ipUserList.stream().filter(ipUser -> !oldIpUserSet.contains(ipUser)).collect(Collectors.toList());
        List<IpUser> delIpUserList =
            oldIpUserList.stream().filter(ipUser -> !ipUserSet.contains(ipUser)).collect(Collectors.toList());
    ...
    public List<IpUserVO> getIpUserVOListByIpId(Long ipId) {
        ...
        //获取用户部门Id和部门名称映射
        Map<Long, String> deptIdNameMap = deptService.getDeptIdAndNameMapping(
    ...
    private List<SysUser> getActiveUserIdList(Long ipId) {
        List<Long> userIdList =
            ipUserMapper.selectList(new LambdaQueryWrapper<IpUser>().eq(IpUser::getIpId, ipId)).stream()
                .map(IpUser::getUserId).collect(Collectors.toList());
        if (userIdList.isEmpty()) {
            return Collections.emptyList();
        }
        //获取知识产权相关有效成员集合
        return sysUserService.filterActiveUserIdList(userIdList);
    }
