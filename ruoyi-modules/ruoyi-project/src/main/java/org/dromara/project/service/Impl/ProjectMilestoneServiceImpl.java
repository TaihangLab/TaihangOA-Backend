...
import org.dromara.project.domain.ProjectMilestoneOss;
...
import org.dromara.project.mapper.ProjectMilestoneMapper;
...
public class ProjectMilestoneServiceImpl implements ProjectMilestoneService {
    ...
    public int deleteMilestoneByProjectId(Long projectId) {
        ...
        List<Long> milestoneIds =
            projectMilestones.stream().map(ProjectMilestone::getMilestoneId).collect(Collectors.toList());

        // 2. 使用大事记 ID 删除每个大事记对应的 OSS 对象
        if (!milestoneIds.isEmpty()) {
            projectMilestoneOssMapper.delete(
                new LambdaQueryWrapper<ProjectMilestoneOss>().in(ProjectMilestoneOss::getMilestoneId, milestoneIds));
        ...
        // 3. 删除该项目下的所有大事记
        return projectMilestoneMapper.delete(
            new LambdaQueryWrapper<ProjectMilestone>().eq(ProjectMilestone::getProjectId, projectId));
    ...
    public int deleteProjectMilestone(Long milestoneId) {
        ...
        long count1 = projectMilestoneOssMapper.selectCount(
            new LambdaQueryWrapper<ProjectMilestoneOss>().eq(ProjectMilestoneOss::getMilestoneId, milestoneId));
        ...
        Long count2 = projectMilestoneTypeMapper.selectCount(
            new LambdaQueryWrapper<ProjectMilestoneType>().eq(ProjectMilestoneType::getMilestoneId, milestoneId));
        if (count2 > 0) {
            projectMilestoneTypeMapper.delete(
                new LambdaQueryWrapper<ProjectMilestoneType>().eq(ProjectMilestoneType::getMilestoneId, milestoneId));
        }
        ...
        return projectMilestoneMapper.delete(
            new LambdaQueryWrapper<ProjectMilestone>().eq(ProjectMilestone::getMilestoneId, milestoneId));
    }
    ...
    public List<ProjectMilestoneVo> queryMilestoneList(ProjectMilestoneBo projectMilestoneBo) {
        ...
        if (projectMilestoneBo.getMilestoneType() != null) {
            milestoneIds = projectMilestoneTypeMapper.selectList(
                    new LambdaQueryWrapper<ProjectMilestoneType>().eq(ProjectMilestoneType::getMilestoneType,
                        projectMilestoneBo.getMilestoneType()).select(ProjectMilestoneType::getMilestoneId)).stream()
                .map(ProjectMilestoneType::getMilestoneId).collect(Collectors.toList());
        }
        ...
        lambdaQueryWrapper
            ...
            .and(wrapper -> wrapper.apply("1=1")
                .like(StringUtils.isNotBlank(projectMilestoneBo.getKeyword()), ProjectMilestone::getMilestoneTitle,
                    projectMilestoneBo.getKeyword()).or()
                .like(StringUtils.isNotBlank(projectMilestoneBo.getKeyword()), ProjectMilestone::getMilestoneRemark,
                    projectMilestoneBo.getKeyword()))
            .ge(projectMilestoneBo.getMilestoneStaTime() != null, ProjectMilestone::getMilestoneDate,
                projectMilestoneBo.getMilestoneStaTime())
            .le(projectMilestoneBo.getMilestoneEndTime() != null, ProjectMilestone::getMilestoneDate,
                projectMilestoneBo.getMilestoneEndTime())
    ...
    private List<ProjectMilestoneVo> buildMilestoneVos(List<ProjectMilestone> milestones) {
        ...
        for (ProjectMilestone milestone : milestones) {
            ...
            List<Long> ossIds = projectMilestoneOssMapper.selectList(
                    new LambdaQueryWrapper<ProjectMilestoneOss>().eq(ProjectMilestoneOss::getMilestoneId,
                        milestone.getMilestoneId())).stream().map(ProjectMilestoneOss::getOssId).filter(Objects::nonNull)
    ...
    private Set<ProjectMilestoneTypeEnum> getCategoryEnumsByMilestoneId(Long milestoneId) {
        return projectMilestoneTypeMapper.selectList(
                new LambdaQueryWrapper<ProjectMilestoneType>().eq(ProjectMilestoneType::getMilestoneId, milestoneId))
            .stream().map(ProjectMilestoneType::getMilestoneType).collect(Collectors.toCollection(TreeSet::new));
    }
    ...
    public Set<ProjectMilestoneTypeEnum> getCategoryEnumsByProjectId(Long projectId) {
        ...
        Set<Long> milestoneIds = projectMilestoneMapper.selectList(
                new LambdaQueryWrapper<ProjectMilestone>().eq(ProjectMilestone::getProjectId, projectId)).stream()
            .map(ProjectMilestone::getMilestoneId).collect(Collectors.toSet());
    ...
    private List<ProjectMilestone> fetchProjectMilestones(ProjectMilestoneBo projectMilestoneBo) {
        ...
        milestoneLambdaQueryWrapper.eq(projectMilestoneBo.getProjectId() != null, ProjectMilestone::getProjectId,
            projectMilestoneBo.getProjectId());
    ...
    private List<ProjectMilestoneOss> fetchMilestoneOssList(List<ProjectMilestone> milestones) {
        List<Long> milestoneIds =
            milestones.stream().map(ProjectMilestone::getMilestoneId).collect(Collectors.toList());
    ...
    // 构建 OSS ID 到大事记名称的映射
    private Map<Long, String> buildOssIdToMilestoneNameMap(List<ProjectMilestoneOss> milestoneOssList,
        Map<Long, String> milestoneIdToNameMap) {
    ...
    private Page<SysOssVo> fetchOssPage(Map<Long, String> ossIdToMilestoneNameMap, PageQuery pageQuery) {
        ...
        ossLambdaQueryWrapper.in(SysOss::getOssId, ossIds).orderByDesc(SysOss::getCreateTime);
