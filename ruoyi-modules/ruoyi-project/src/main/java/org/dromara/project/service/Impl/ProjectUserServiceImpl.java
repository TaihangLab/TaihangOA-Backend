...
import org.dromara.project.mapper.ProjectBaseInfoMapper;
import org.dromara.project.mapper.ProjectUserMapper;
...
public class ProjectUserServiceImpl implements ProjectUserService {
    ...
    private void setProjectLevelCount(ProjectUserVo projectUserVo, Long userId, boolean isCurrent) {
        List<Map<String, Object>> mapList =
            isCurrent ? getNowProjectLevelCountByUserId(userId) : getProjectLevelCountByUserId(userId);
    ...
    public void updateProjectUsers(List<ProjectUserBo> projectUserBOList, Long projectId) {
        insertLoginUserToBO(projectUserBOList);
        List<ProjectUser> oldProjectUserList = projectUserMapper.selectList(
        ...
        List<ProjectUser> addProjectUserList =
            newProjectUserList.stream().filter(projectUser -> !oldProjectUserSet.contains(projectUser))
                .collect(Collectors.toList());
        List<ProjectUser> delProjectUserList =
            oldProjectUserList.stream().filter(projectUser -> !newProjectUserSet.contains(projectUser))
                .collect(Collectors.toList());
    ...
    private Set<Long> getUserIdsByProjectId(Long projectId) {
        ...
        // 获取与项目相关的 SysUser 对象
        List<SysUser> sysUsers = projectUsers.stream().map(ProjectUser::getUserId) // 获取用户 ID 列表
     ...
     */
    private List<ProjectUserVo> buildProjectUserVoList(Set<Long> userIds, Map<Long, SysUser> userIdToUserMap,
        Map<Long, String> deptIdToNameMap, Map<Long, List<ProjectUserRoleEnum>> userIdToProjectUserRolesMap) {
        ...
        for (Long userId : userIds) {
            ...
            List<ProjectUserRoleEnum> projectUserRoleEnums =
                userIdToProjectUserRolesMap.getOrDefault(userId, Collections.singletonList(ProjectUserRoleEnum.UNKNOWN))
                    .stream().filter(
                        projectUserRoleEnum -> !projectUserRoleEnum.equals(ProjectUserRoleEnum.PROJECT_ENTRY_OPERATOR))
                    .collect(Collectors.toList());
    ...
    public TableDataInfo<ProjectUserVo> queryPageAllList(ProjectUserBo projectUserBo, PageQuery pageQuery) {
        ...
        List<ProjectUserVo> projectUserVoList =
            userPage.getRecords().stream().map(this::createProjectUserVo).collect(Collectors.toList());
    ...
    private Map<ProjectLevelEnum, Integer> processProjectLevelCount(List<Map<String, Object>> mapList) {
        ...
        for (Map<String, Object> map : mapList) {
            ...
            ProjectLevelEnum level =
                Arrays.stream(ProjectLevelEnum.values()).filter(e -> e.getValue().equals(projectLevelValue)).findFirst()
                    .orElse(null);
    ...
    private void setProjectLevelDataToVo(ProjectUserVo projectUserVo,
        Map<ProjectLevelEnum, Integer> projectLevelCountMap, boolean isCurrent) {
        if (isCurrent) {
            ...
            projectUserVo.setUserProjectNumNow(
                projectLevelCountMap.values().stream().mapToInt(Integer::intValue).sum());
        } else {
    ...
    public ProjectUserDetailVo getProjectUserDetailById(Long userId) {
        Set<Long> projectIds =
            projectUserMapper.selectList(new LambdaQueryWrapper<ProjectUser>().eq(ProjectUser::getUserId, userId))
                .stream().filter(
                    projectUser -> !projectUser.getProjectUserRole().equals(ProjectUserRoleEnum.PROJECT_ENTRY_OPERATOR))
                .map(ProjectUser::getProjectId).collect(Collectors.toSet());

        if (projectIds.isEmpty()) {
            return new ProjectUserDetailVo();
        }

        List<ProjectBaseInfo> projectBaseInfos = projectBaseInfoMapper.selectList(
            new LambdaQueryWrapper<ProjectBaseInfo>().in(ProjectBaseInfo::getProjectId, projectIds)
        ...
        Map<ProjectLevelEnum, List<ProjectBaseInfo>> classifiedProjects =
            projectBaseInfos.stream().collect(Collectors.groupingBy(ProjectBaseInfo::getProjectLevel));

        ProjectUserDetailVo projectUserDetailVo = new ProjectUserDetailVo();
        projectUserDetailVo.setNationProjectBaseInfos(
            classifiedProjects.getOrDefault(ProjectLevelEnum.NATIONAL, Collections.emptyList()));
        projectUserDetailVo.setProvincialProjectBaseInfos(
            classifiedProjects.getOrDefault(ProjectLevelEnum.PROVINCIAL, Collections.emptyList()));
        projectUserDetailVo.setEnterpriseProjectBaseInfos(
            classifiedProjects.getOrDefault(ProjectLevelEnum.ENTERPRISE, Collections.emptyList()));
    ...
    public void deleteProjectUserList(List<ProjectUser> projectUserList) {
        ...
        projectUserList.forEach(projectUser -> projectUserMapper.delete(
            new LambdaQueryWrapper<ProjectUser>().eq(ProjectUser::getProjectId, projectUser.getProjectId())
    ...
    public Map<String, String> getTitleAndNameMapping(Long projectId) {
        ...
        return projectUserList.stream().collect(
            Collectors.groupingBy(projectUser -> projectUser.getProjectUserRole().getTitle(),
                Collectors.mapping(projectUser -> userIdToUserMap.get(projectUser.getUserId()).getNickName(),
                    Collectors.joining(","))));
    }

    private List<ProjectUser> projectUserBOListToProjectUserList(List<ProjectUserBo> projectUserBOList,
        Long projectId) {
        if (projectUserBOList == null || projectUserBOList.isEmpty()) {
            return Collections.emptyList();
        }
        return projectUserBOList.stream().flatMap(projectUserBo -> {
            List<ProjectUserRoleEnum> roles = projectUserBo.getProjectUserRoleList();
            if (roles == null || roles.isEmpty()) {
                return Stream.empty();
            }
            return roles.stream().map(projectUserRoleEnum -> {
    ...
    private void insertLoginUserToBO(List<ProjectUserBo> projectUserBoList) {
        ...
        projectLoginUserBo.setProjectUserRoleList(
            Collections.singletonList(ProjectUserRoleEnum.PROJECT_ENTRY_OPERATOR));
