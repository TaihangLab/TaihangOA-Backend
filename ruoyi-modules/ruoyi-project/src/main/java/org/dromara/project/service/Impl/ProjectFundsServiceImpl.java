...
import org.dromara.project.domain.bo.ProjectFundsBO;
...
public class ProjectFundsServiceImpl implements ProjectFundsService {
    ...
    public ProjectFundsVO selectProjectFundsVOById(Long projectId) {
        return projectFundsMapper.selectVoOne(
    ...
    private void synchronizeFundsToBalance(ProjectFunds projectFunds) {
        Map<String, Object> fundsMap = BeanUtil.beanToMap(projectFunds);
    ...
    public Map<Long, ProjectFunds> getProjectFundsMapByProjectIdList(List<Long> projectIdList) {
        return projectFundsMapper.selectList(
                new LambdaQueryWrapper<ProjectFunds>().in(ProjectFunds::getProjectId, projectIdList)).stream()
            .collect(Collectors.toMap(ProjectFunds::getProjectId, projectFunds -> projectFunds));
    }
    ...
    public ProjectFunds getProjectFundsMapByProjectId(Long projectId) {
        return projectFundsMapper.selectOne(
            new LambdaQueryWrapper<ProjectFunds>().eq(ProjectFunds::getProjectId, projectId));
    }
    ...
    private void updateBalance(ProjectFunds projectFunds, ProjectFunds projectFundsOld,
        ...
        Map<String, Object> fundsMap = BeanUtil.beanToMap(projectFunds);
        Map<String, Object> fundsOldMap = BeanUtil.beanToMap(projectFundsOld);
        fundsMap.forEach((key, value) -> {
            if (fundsMapping.containsKey(key)) {
                BigDecimal amount = (BigDecimal)value;
