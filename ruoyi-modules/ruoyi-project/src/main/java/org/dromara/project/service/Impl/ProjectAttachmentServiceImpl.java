...
import org.dromara.project.mapper.ProjectAttachmentMapper;
import org.dromara.project.service.ProjectAttachmentService;
...
public class ProjectAttachmentServiceImpl implements ProjectAttachmentService {
    ...
    public void deleteAllProjectAttachmentByProID(Long projectId) {
        projectAttachmentMapper.delete(
            (new LambdaQueryWrapper<ProjectAttachment>()).eq(ProjectAttachment::getProjectId, projectId));
    }
    ...
    public List<SysOssVo> selectSysOssVOListByProId(Long projectId) {
        List<Long> ossIds = projectAttachmentMapper.selectList(
                (new LambdaQueryWrapper<ProjectAttachment>()).eq(ProjectAttachment::getProjectId, projectId)).stream()
            .map(ProjectAttachment::getOssId).collect(Collectors.toList());
        if (ossIds.isEmpty()) {
    ...
    public void updateProjectAttachmentList(List<Long> ossIds, Long projectId) {
        List<ProjectAttachment> oldProjectAttachments = projectAttachmentMapper.selectList(
            new LambdaQueryWrapper<ProjectAttachment>().eq(ProjectAttachment::getProjectId, projectId));
        ...
        List<ProjectAttachment> addProjectAttachmentList = newProjectAttachments.stream()
            .filter(projectAttachment -> !oldProjectAttachmentSet.contains(projectAttachment))
            .collect(Collectors.toList());
        List<ProjectAttachment> delProjectAttachmentList = oldProjectAttachments.stream()
            .filter(projectAttachment -> !newProjectAttachmentSet.contains(projectAttachment))
            .collect(Collectors.toList());
        ...
        if (!delProjectAttachmentList.isEmpty()) {
            deleteProjectAttachmentByIdList(
                delProjectAttachmentList.stream().map(ProjectAttachment::getId).collect(Collectors.toList()));
        }
    }
