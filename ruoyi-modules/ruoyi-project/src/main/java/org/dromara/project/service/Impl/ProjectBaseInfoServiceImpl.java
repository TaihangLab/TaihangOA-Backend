...
import org.dromara.project.domain.vo.ProjectInfoVO;
...
import static org.dromara.common.core.constant.IpConstants.UNASSOCIATED_PROJECT_CODE;
import static org.dromara.common.core.constant.IpConstants.UNASSOCIATED_PROJECT_IDENTIFIER;
...
public class ProjectBaseInfoServiceImpl implements ProjectBaseInfoService {
    ...
    private Set<ProjectLevelEnum> getAllProjectLevels() {
        return projectBaseInfoMapper.selectList().stream().map(ProjectBaseInfo::getProjectLevel)
    ...
    private List<ProjectBaseInfo> getProjectsByLevel(ProjectLevelEnum projectLevel) {
        return projectBaseInfoMapper.selectList(
            new LambdaQueryWrapper<ProjectBaseInfo>().eq(ProjectBaseInfo::getProjectLevel, projectLevel));
    }
    ...
    private LambdaQueryWrapper<ProjectBaseInfo> buildMyListQueryWrapper(ProjectBaseInfoBO projectBaseInfoBO) {
        ...
        List<Long> loginProjectIdList =
            Optional.ofNullable(LoginHelper.getUserId()).map(projectUserService::getProjectIdListByUserId)
    ...
    private void buildRecords(List<ProjectBaseInfoVO> projectBaseInfoVOList) {
        ...
        List<Long> projectIdList =
            projectBaseInfoVOList.stream().map(ProjectBaseInfoVO::getProjectId).collect(Collectors.toList());
        //获取经费对应信息
    ...
    private LambdaQueryWrapper<ProjectBaseInfo> buildCommonQueryWrapper(ProjectBaseInfoBO projectBaseInfoBO) {
        ...
        lqw.like(StringUtils.isNotBlank(projectBaseInfoBO.getAssignedSubjectName()),
            ProjectBaseInfo::getAssignedSubjectName, projectBaseInfoBO.getAssignedSubjectName());
        lqw.like(StringUtils.isNotBlank(projectBaseInfoBO.getAssignedSubjectSection()),
            ProjectBaseInfo::getAssignedSubjectSection, projectBaseInfoBO.getAssignedSubjectSection());
        //log.info("projectBaseInfoBO.getHasCooperativeUnit()为:{}",projectBaseInfoBO.getHasCooperativeUnit().getValue());
        lqw.eq(projectBaseInfoBO.getHasCooperativeUnit() != null, ProjectBaseInfo::getHasCooperativeUnit,
            projectBaseInfoBO.getHasCooperativeUnit());
        lqw.eq(projectBaseInfoBO.getProjectLevel() != null, ProjectBaseInfo::getProjectLevel,
            projectBaseInfoBO.getProjectLevel());
        lqw.ge(projectBaseInfoBO.getProjectEstablishTimeSta() != null, ProjectBaseInfo::getProjectEstablishTime,
            projectBaseInfoBO.getProjectEstablishTimeSta());
        lqw.le(projectBaseInfoBO.getProjectEstablishTimeEnd() != null, ProjectBaseInfo::getProjectEstablishTime,
            projectBaseInfoBO.getProjectEstablishTimeEnd());
        lqw.ge(projectBaseInfoBO.getProjectScheduledCompletionTimeSta() != null,
            ProjectBaseInfo::getProjectScheduledCompletionTime,
            projectBaseInfoBO.getProjectScheduledCompletionTimeSta());
        lqw.le(projectBaseInfoBO.getProjectScheduledCompletionTimeEnd() != null,
            ProjectBaseInfo::getProjectScheduledCompletionTime,
            projectBaseInfoBO.getProjectScheduledCompletionTimeEnd());
    ...
    private void setUsers(ProjectBaseInfoVO projectBaseInfoVO, Long projectId) {
        ...
        projectBaseInfoVO.setProjectLeader(
            titleAndNameMapping.getOrDefault(ProjectUserRoleEnum.PROJECT_LEADER.getTitle(), ""));
        projectBaseInfoVO.setCompanyLeader(
            titleAndNameMapping.getOrDefault(ProjectUserRoleEnum.COMPANY_LEADER.getTitle(), ""));
        projectBaseInfoVO.setDepartmentLeader(
            titleAndNameMapping.getOrDefault(ProjectUserRoleEnum.DEPARTMENT_LEADER.getTitle(), ""));
        projectBaseInfoVO.setResearchManager(
            titleAndNameMapping.getOrDefault(ProjectUserRoleEnum.RESEARCH_MANAGER.getTitle(), ""));
    }
}
