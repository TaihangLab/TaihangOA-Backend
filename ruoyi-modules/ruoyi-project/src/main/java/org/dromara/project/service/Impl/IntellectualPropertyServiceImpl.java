...
import org.dromara.project.mapper.IntellectualPropertyMapper;
...
import static org.dromara.common.core.constant.IpConstants.*;
...
public class IntellectualPropertyServiceImpl implements IntellectualPropertyService {
    ...
    public IntellectualPropertyDetailVO getIntellectualPropertyDetail(Long ipId) {
        ...
        if (UNASSOCIATED_PROJECT_CODE.equals(intellectualProperty.getProjectId())) {
            ...
            intellectualPropertyDetailVO.setAssignedSubjectName(Optional.ofNullable(
                    projectBaseInfoService.selectProjectBaseInfoById(intellectualPropertyDetailVO.getProjectId()))
                .map(ProjectBaseInfo::getAssignedSubjectName).orElse(PROJECT_DELETED_REASSOCIATE));
        }
        intellectualPropertyDetailVO.setProjectPath(
            projectBaseInfoService.getProjectPathById(intellectualProperty.getProjectId()));
    ...
    public TableDataInfo<IntellectualPropertyVO> queryIntellectualPropertVOList(
        IntellectualPropertyBO intellectualPropertyBO, PageQuery pageQuery) {
    ...
    public Map<String, Integer> getIpTypeStatistics() {
        ...
        // 流处理知识产权并更新计数。
        intellectualPropertyMapper.selectList(Wrappers.lambdaQuery()).stream().filter(ip -> ip.getIpType() != null)
    ...
    private LambdaQueryWrapper<IntellectualProperty> buildIntellectualPropertyQueryWrapper(
        ...
        lqw.eq(intellectualPropertyBO.getProjectId() != null, IntellectualProperty::getProjectId,
                intellectualPropertyBO.getProjectId())
            .eq(intellectualPropertyBO.getIpType() != null, IntellectualProperty::getIpType,
                intellectualPropertyBO.getIpType())
            .eq(intellectualPropertyBO.getIpStatus() != null, IntellectualProperty::getIpStatus,
                intellectualPropertyBO.getIpStatus())
            .like(StringUtils.isNotBlank(intellectualPropertyBO.getIpName()), IntellectualProperty::getIpName,
                intellectualPropertyBO.getIpName())
            .ge(intellectualPropertyBO.getIpDateSta() != null, IntellectualProperty::getIpDate,
                intellectualPropertyBO.getIpDateSta())
            .le(intellectualPropertyBO.getIpDateEnd() != null, IntellectualProperty::getIpDate,
                intellectualPropertyBO.getIpDateEnd()).orderByDesc(IntellectualProperty::getUpdateTime);
    ...
    private void setAssignedSubjectName(List<IntellectualPropertyVO> recordList) {
        Set<Long> projectSet =
            recordList.stream().map(IntellectualPropertyVO::getProjectId).collect(Collectors.toSet());
        Map<Long, String> projectIdAndNameMapping =
            projectBaseInfoService.getProjectIdAndNameMappingByProjectIdSet(projectSet);

        recordList.forEach(intellectualPropertyVO -> intellectualPropertyVO.setAssignedSubjectName(
    ...
    private List<Long> getIpIdListFromUserId(Long userId) {
        return Optional.ofNullable(userId).map(ipUserService::getIpUserListByUserId).orElseGet(Collections::emptyList)
            .stream().map(IpUser::getIpId).collect(Collectors.toList());
    }
}
